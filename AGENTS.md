...
- Session start: run `context` to see active goals and `list` for the hierarchy. State lives in `BTASK_DB` (default `.btask/btask.db` under your cwd).
- Larger goal (a spec, a ticket batch): `create` a goal first, or attach to a matching open goal instead of duplicating it.
- One sub-task per unit of work (`create --parent <goal-id>`); set it `in_progress` when starting and `finished` when done. Never leave a task `in_progress` at session end.
